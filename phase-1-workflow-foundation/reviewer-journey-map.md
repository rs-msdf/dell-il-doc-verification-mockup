...
| 4. Review expected documents | Confirm required document coverage for the group. | Expected document names, current document states, selected document item, verification indicators, applicant comments, uploaded-file selector where available. | Decide which document item needs attention. | Required document is `Not uploaded`, `Reopened`, or otherwise not ready for verification. |
...
| 7. Identify missing decisions | See what prevents the group from completing. | Focused status row or blocker display with unverified document counts, unchecked field counts, no-preview context, and required reopen comments when relevant. | Decide which blocker to resolve next. | Missing work is only visible after manually scanning document or field rows. |
...
| 9. Move to another group | Continue review while preserving overall awareness. | Group overview/navigation and application-level group progress. | Select another incomplete group or revisit a completed group. | Reviewer loses context after switching groups. |
